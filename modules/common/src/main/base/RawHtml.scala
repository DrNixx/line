...
final object RawHtml {
  ...
  private[this] val urlPattern = (
    ...
    """(?<![/@.-].)(?:\w{1,15}+\.){1,3}(?>com|org|edu))""" + // "live.chess-online.com", etc
    """([/?#][-–—\w/.~!$&'()*+,;=:#?@%]{0,300}+)?""" + // path, params
    """(?![\w/~$&*+=#@%])""" // neg lookahead
  ).r.pattern
  ...
  private[this] final val DOMAIN = "live.chess-online.com"
  ...
  // No: contact@live.chess-online.com, @1, http://example.com/@happy0, @lichess.org
