  ...
  private def registerRageSit(record: UserRecord, delta: Int): Funit = {
    ...
    (delta < 0) ?? {
      if (record.rageSit.isTerrible) {
        lila.log("ragesit").warn(s"Close https://live.chess-online.com/@/${record.userId} ragesit=${record.rageSit}")
        ...
        case (m, u) =>
          lila.log("ragesit").info(s"https://live.chess-online.com/@/${u.id} ${record.rageSit.counterView}")
