...
final private[round] class RoundDuct(
  ...
  val process: Duct.ReceiveAsync = {
        ...
        gameRepo hasHoldAlert pov flatMap {
          ...
          case false =>
            lila
              .log("cheat")
              .info(
                s"hold alert $ip https://live.chess-online.com/${pov.gameId}/${pov.color.name}#${pov.game.turns} ${pov.player.userId | "anon"} mean: $mean SD: $sd"
              )
    ...
    case WsBoot =>
      handle { game =>
        game.playable ?? {
          messenger.system(game, "Chess-Online has been updated! Sorry for the inconvenience.")
