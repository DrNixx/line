  ...
  val process: Duct.ReceiveAsync = {
    ...
    case Protocol.In.HoldAlert(fullId, ip, mean, sd) => handle(fullId.playerId) { pov =>
      lila.game.GameRepo hasHoldAlert pov flatMap {
        case true => funit
        case false =>
          lila.log("cheat").info(s"hold alert $ip https://live.chess-online.com/${pov.gameId}/${pov.color.name}#${pov.game.turns} ${pov.player.userId | "anon"} mean: $mean SD: $sd")
    ...
    case DeployPost => handle { game =>
      game.playable ?? {
        ...
        messenger.system(game, (_.untranslated("Chess-Online has been updated! Sorry for the inconvenience.")))
