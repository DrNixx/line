...
final class SlackApi(
    ...
    client(
      SlackMessage(
        ...
        text = {
          ...
          s"checked out _*${userLink(user.id)}*_'$finalS communications "
  ...
  private def userLink(user: User): String            = userLink(user.id)
      ...
      .map {
        case None =>
          SlackMessage(
            username = mod.username,
            icon = "eyes",
            text = s"Let's have a look at _*${userLink(user.id)}*_",
            channel = rooms.tavern
          )
        case Some(note) =>
          SlackMessage(
            ...
            text = s"_*${userLink(user.id)}*_ (${userNotesLink(user.id)}):\n" +
