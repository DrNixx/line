...
lazy val common = smallModule("common",
  ...
  Seq(
    ...
    kamon.core, scalatags, jodaForms, scaffeine, specs2, apacheText, icu4j) ++ reactivemongo.bundle
)
...
lazy val oidc = smallModule("oidc",
  Seq(common, db, user),
  Seq(autoconfig) ++ nimbusds.bundle ++ macwire.bundle
)

lazy val security = module("security",
  Seq(common, hub, db, user, i18n, slack, oauth, oidc),
