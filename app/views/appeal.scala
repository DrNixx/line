          ...
          inquiry match {
            ...
            case Some(Inquiry(mod, _)) if ctx.userId has mod =>
              frag(
                ...
                if (appeal.isOpen)
                  frag(
                    postForm(action := routes.Appeal.act(suspect.user.id, "close"))(
                      submitButton("Close")(cls := "button button-red button-thin")
                    ),
                    if (appeal.isMuted)
                      postForm(action := routes.Appeal.act(suspect.user.id, "open"))(
                        submitButton("Un-mute")(cls := "button button-green button-thin")
                      )
                    else
                      postForm(action := routes.Appeal.act(suspect.user.id, "mute"))(
