...
object Game extends LilaController {
  ...
  def exportByUser(userId: UserModel.ID) = OpenOrScoped()(
    open = ctx => handleExport(userId, ctx.me, ctx.req, oauth = false),
    scoped = req => me => handleExport(userId, me.some, req, oauth = true)
  )

  def apiExportByUser(userId: UserModel.ID) = AnonOrScoped()(
    anon = req => handleExport(userId, none, req, oauth = false),
    scoped = req => me => handleExport(userId, me.some, req, oauth = true)
  )
