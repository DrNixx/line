...
final class Team(env: Env) extends LilaController(env):
  ...
  private def renderEdit(team: TeamModel, form: Form[?])(using me: Me, ctx: Context) = for member <-
      env.team.memberRepo.get(team.id, me)
                    ...
                    .sequentially: change =>
                      env.msg.api.systemPost(
                        ...
                        lila.msg.MsgPreset.newPermissions(
                          if asMod then LightUser.lichess() else me.light,
                          team.team.light,
